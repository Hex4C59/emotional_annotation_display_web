...
class EmotionLabelingApp {
    ...
    async initApp() {
        ...
        console.log('Starting 9-point scale initialization with user:', this.userManager.getCurrentUsername());
        ...
        Promise.all([
            ...
            console.log('9-point scale initialization completed successfully');
        }).catch(error => {
    ...
    initModules(elements) {
        ...
        // 初始化音频列表管理器（9点量表版本）
        this.audioListManager = new AudioListManager(elements.speakerSelect, elements.audioListContainer, '9_point');
    ...
    setupEventListeners(elements) {
        ...
        // 切换量表按钮事件
        const switchTo5PointButton = document.getElementById('switch-to-5point');
        if (switchTo5PointButton) {
            switchTo5PointButton.addEventListener('click', () => this.handleSwitchTo5Point());
        }
    ...
    async handleAudioSelect(audioFile, index) {
        ...
        // 加载音频
        await this.loadAudioFile(audioFile, index);
    ...
    async loadAudioFile(audioFile, index) {
        ...
        try {
            ...
            if (audioFile.labeled) {
                ...
                try {
                    ...
                    if (labelData && labelData.success && labelData.data) {
                        ...
                        if (label.va_scale && label.va_scale !== '9_point') {
                            console.log('This is 5-point scale data, not loading for 9-point interface');
                            ...
                            if (label.emotion_type) {
                                ...
                                if (this.emotionAnnotator.emotionType === 'non-neutral' && label.discrete_emotion) {
                                    ...
                                    console.log('Loading discrete emotions in main_9point.js:', emotions);
                                    this.emotionAnnotator.selectedDiscreteEmotions = emotions;
                        ...
                        console.warn(`optimizedDataService.getLabel returned no data for ${audioFile.file_name}`);
                        this.updateSaveButtonStatus(false);
                    }
                } catch (error) {
                ...
                console.log(`File ${audioFile.file_name} is not marked as labeled.`);
                this.updateSaveButtonStatus(false);
            }
    ...
    async handleSaveVa() {
        ...
        try {
            ...
            // 使用保存优化器进行异步保存
            const saveKey = `va_${currentAudio.speaker}_${currentAudio.file_name}_${this.userManager.getCurrentUsername()}`;
            const result = await new Promise((resolve, reject) => {
                window.saveOptimizer.optimizedSave(saveKey, labelData, (success, error) => {
                    if (success) {
                        resolve({ success: true });
                    } else {
                        reject(new Error(error || '保存失败'));
                    }
                });
            });
    ...
    async handleSaveDiscrete() {
        ...
        try {
            ...
            // 使用保存优化器进行异步保存
            const saveKey = `discrete_${currentAudio.speaker}_${currentAudio.file_name}_${this.userManager.getCurrentUsername()}`;
            const result = await new Promise((resolve, reject) => {
                window.saveOptimizer.optimizedSave(saveKey, labelData, (success, error) => {
                    if (success) {
                        resolve({ success: true });
    ...
    handleNext() {
        ...
        if (currentAudio && currentAudio.labeled && Array.isArray(currentAudio.annotation_completeness)) {
            ...
            // 如果没有标注数据，检查当前界面状态
            const completeness = this.emotionAnnotator.getAnnotationCompleteness();
            if (completeness.includes('discrete_complete') || completeness.includes('va_complete')) {
                this.audioListManager.nextAudio();
            } else {
                alert('Please complete the annotation for the current audio before continuing');
            }
    ...
    showCompletionPrompt(data) {
        ...
        document.getElementById('continueToOtherScale').addEventListener('click', () => {
            ...
            // 跳转到另一个量表
            const currentScale = data.current_scale || '9_point';
            if (currentScale === '9_point') {
                // 当前是9点量表，跳转到5点量表
     ...
     */
    showSwitchButton() {
        const switchButton = document.getElementById('switch-to-5point');
        if (switchButton) {
    ...
    handleSwitchTo5Point() {
        ...
        console.log('Switching from 9-point to 5-point scale');
    ...
    cleanupBeforeSwitch() {
        ...
        if (window.optimizedDataService && this.userManager) {
            ...
            if (window.cacheManager) {
                window.cacheManager.clearPattern('va_scale=9_point');
            }
        }
    }
}
